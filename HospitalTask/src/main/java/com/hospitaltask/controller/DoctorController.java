...
public class DoctorController {
	...
	// todo ......under Working ......
	/*
	 * @PutMapping("/save-email/{email}") public ResponseEntity<?>
	 * updateByEmail(@RequestBody Doctor doctor, @PathVariable String email) {
	 * Doctor doctorCheck = doctorRepo.findByEmail(email); if
	 * (!doctorCheck.equals(null)) { doctorService.updateDoctorByEmail(doctor,
	 * email); return new ResponseEntity<>("Doctor Update By :  " + email +
	 * doctor.getEmail(), HttpStatus.OK); } else return new
	 * ResponseEntity<>("Doctor Not Available This Email :" + email,
	 * HttpStatus.NOT_FOUND); }
	 * 
	 * @PutMapping("/save-name/{name}") public ResponseEntity<?>
	 * updateByName(@RequestBody Doctor doctor, @PathVariable String name) { Doctor
	 * doctorCheck = doctorRepo.findByName(name); if (!doctorCheck.equals(null)) {
	 * doctorService.updateDoctorByEmail(doctor, name); return new
	 * ResponseEntity<>("Doctor Update By :  " + name + doctor.getEmail(),
	 * HttpStatus.OK); } else return new
	 * ResponseEntity<>("Doctor Not Available This Email :" + name,
	 * HttpStatus.NOT_FOUND); }
	 */
	...
	public ResponseEntity<?> findByEmailId(@PathVariable String id) {
		...
		if (doctor == null)
			return CustomResponseHandler.response(RECORD_NOT_FOUND, HttpStatus.NOT_FOUND, id);
		return CustomResponseHandler.response("Record Found Success", HttpStatus.OK,
				this.doctorService.findByEmail(id));
	}
