...
public class LoginController {
    ...
    public ResponseEntity<?> login(@RequestBody @NotNull AuthRequest authRequest) throws Exception {
        ...
        if (doctor != null && doctor.getIsActive() && doctor.getRoles().getId()==2){
            ;
        }
        else {
            ...
            if (patient != null && patient.getIsActive() && patient.getRoles().getId() == 3) {
            } else {
                ...
                if (superAdmin != null && superAdmin.getIsActive() && superAdmin.getRoles().getId()== 1) {
                    SuperAdmin superAdmin1 = new SuperAdmin();
                    superUserCheck(superAdmin1);
                } else {
    ...
    public void superUserCheck(SuperAdmin superAdmin) {

        List<SuperAdmin> superAdminFetchData = superAdminRepo.findAll();
        if (superAdminFetchData.size() == 0) {

            superAdmin.setId(1);
            superAdmin.setEmail("User1@gmail.com");
